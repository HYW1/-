...
- 独立智能选股中心：默认 A 股优先，候选池按当日主力资金/涨幅动态补入，再按严进策略每日重排
- “严格推荐”同时要求策略评分 ≥ 64、未触发 RSI/乖离率追高限制、风险收益比 ≥ 2:1；无结果时明确提示而不降低硬规则
- 可组合量化规则：三连阳、四连阳、多头排列、MACD 多头、20 日突破、温和放量、缩量回踩、靠近 MA20、低波动、动量适中
- 内置连阳接力、趋势共振、放量突破、缩量回踩、低波稳健组合，也可自定义“全部满足 / 任一满足”
- 市况自适应：进攻优先趋势/放量，防守优先低波回踩，均衡偏好缩量买点
...
- 三连阳 / 四连阳：连续 3 / 4 个交易日收盘高于开盘，且收盘价逐日抬高
- 多头排列：收盘价 > MA5 > MA20，且 MA20 相比 5 日前向上
- 20 日突破：收盘价突破此前 20 日最高价
- 温和放量：当日上涨且量比位于 1.2–2.5
- 靠近 MA20：收盘价位于 MA20 的 -2% 至 +3%
- 动量适中：RSI 位于 45–70，且 MA5 乖离率绝对值不超过 3%

形态规则与策略评分采用 AND 关系。即使命中连阳或突破，只要 RSI > 80 或 MA5 乖离率 > 5%，仍会被硬规则标记为谨慎。
